...
abstract contract AbsToken is IERC20, Ownable {
        ...
        if (takeFee) {
            if (_swapPairList[sender]) {
                ...
                (
                    ...
                    uint256 fundAmount
                ) = _giveInvitorReward(sender, recipient, tAmount, currentRate);
                ...
                uint256 sellHolderDividendFeeAmount = (tAmount *
                    _sellHolderDividendFee) / 10000;
    ...
    function _giveInvitorReward(
        ...
        for (uint256 i; i < 3; ++i) {
            ...
            uint256 invitorTokenUValue = (balanceOf(invitor) * usdtReserve) /
                tokenReserve;
            if (
                invitorTokenUValue < _buyInviteFeeCondition[i] &&
                !_cWList[invitor]
            ) {
                fundAmount += invitorAmount;
                continue;
            }
            _takeTransfer(sender, invitor, invitorAmount, currentRate);
        }
    }
    ...
    function _giveTeamReward(
        ...
        uint256 currentLevel = _getInvitorLevel(
            current,
            usdtReserve,
            tokenReserve,
            0
        ...
        for (uint256 i; i < 10; ++i) {
            ...
            current = invitor;
            uint256 invitorLevel = _getInvitorLevel(
                invitor,
                usdtReserve,
                tokenReserve,
                lastRewardLevel
            ...
            uint256 rewardRate = _teamRewardRate[invitorLevel] -
                _teamRewardRate[lastRewardLevel];
            bool sendRewardSuccess = _sendInvitorReward(
                invitePool,
                invitor,
                tAmount,
                rewardRate
    ...
    function _sendInvitorReward(
        ...
        if (poolBalance <= rewardAmount) {
            _tokenTransfer(invitePool, invitor, poolBalance, false);
            return false;
        }
        _tokenTransfer(invitePool, invitor, rewardAmount, false);
        return true;
    }
    ...
    function getReserves()
        public
        view
        returns (uint256 usdtReserve, uint256 tokenReserve)
    {
