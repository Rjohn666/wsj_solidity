...
abstract contract AbsToken is IERC20, Ownable {
	...
	function processReward(uint256 gas) private {
		...
		if (
			MossToken.balanceOf(address(this)) < holderRewardCondition ||
			W3nToekn.balanceOf(address(this)) < holderRewardCondition
		) {
			return;
		}
	...
	function _takeTransfer(address sender, address to, uint256 tAmount) private {
		if (!isWalletLimitExempt[to]) {
			require(
				_balances[to] + tAmount <= walletLimit,
	...
	function SwapTokenToFund(uint256 amount) private lockTheSwap {
		...
		IERC20(_usdt).transferFrom(address(token_distributor), address(this), usdt_amount);
	...
	function getBuyAmout(
		uint256 num
	) public view returns (uint256 sellFeeForMoss, uint256 sellFeeForW3n) {
	...
	function setRewardPrams(
		uint256 newHolderRewardCondition,
