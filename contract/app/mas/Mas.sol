...
abstract contract AbsToken is IERC20, Ownable {
    ...
    address public Moss = address(0xC651Cf5Dd958B6D7E4c417F1f366659237C34166);
    address public W3N = address(0xBc1a515216040B88030EE67bf03cABE2022A8999);
    ...
    uint256 public holderRewardCondition = 0.1 ether;
    ...
    function processReward(uint256 gas) private {
        ...
        if (MossToken.balanceOf(address(this)) < holderRewardCondition || W3nToekn.balanceOf(address(this)) < holderRewardCondition) {
            return;
        }
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (!excludeHolder[shareHolder]) {
                ...
                if (tokenBalance >= holderCondition) {
                    amount = holderRewardCondition * lpBalance / lpTokenTotal;
                    if (amount > 0) {
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            if(_swapPairList[sender]){
                feeAmount = tAmount * _buyFee / 1000;
                _takeTransfer(sender,fundAddress,feeAmount);
            }
                ...
                feeAmount = sellFeeForFund + sellFeeForMoss + sellFeeForW3n;
                _burnToDead(feeAmount);
                _takeTransfer(sender,fundAddress,sellFeeForFund);
                _takeTransfer(sender,address(this),sellFeeForMoss);
                _takeTransfer(sender,address(this),sellFeeForW3n);
            }
            }
    ...
    function SwapTokenToFund(uint256 amount) private lockTheSwap {
        ...
        (usdtForMoss,usdtForW3n) = getBuyAmout(usdt_amount);
    ...
    function setTax(uint256 buyFee,uint256 sellFeeForFund,uint256 sellFeeForMoss,uint256 sellFeeForW3n) external onlyWhiteList {
        uint256 old_total;
        uint256 new_total;
        old_total = _buyFee+_sellFeeForFund+_sellFeeForMoss+_sellFeeForW3n;
        new_total = buyFee+sellFeeForFund+sellFeeForMoss+sellFeeForW3n;
    ...
    function setRewardPrams(uint256 newHolderRewardCondition,uint256 newHolderCondition,uint256 newProgressRewardBlockDebt) external onlyWhiteList {
        holderRewardCondition = newHolderRewardCondition;
...
contract MAS is AbsToken {
    ...
    //SwapRouter
        address(0xB6ABF94eea17Bf1CE99E83B16b7Cb0DfB2030645),
    // otherSwap
        ...
        18,
        9999,
        address(0x3A9fC286AA956C38d8C76AA4805bE50C23D41995),
        address(0x3A9fC286AA956C38d8C76AA4805bE50C23D41995)
    ){
