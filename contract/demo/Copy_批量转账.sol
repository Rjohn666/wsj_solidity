...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (isRemoveLP) {
                ...
                swapFeeAmount = tAmount * (_buyFundFee1 + _buyLPFee) / 10000;
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 fundFee1 = _buyFundFee1;
        uint256 lpFee = _buyLPFee;
        uint256 totalFee = fundFee1 + _sellLPDividendFee + lpFee;
        ...
        uint256 fundUsdt = usdtBalance * fundFee1 * 2 / totalFee;
        if (fundUsdt > 0) {
            USDT.transfer(fundAddress, fundUsdt);
        }

        fundUsdt = usdtBalance * lpFee / totalFee;
        if (fundUsdt > 0) {
            address receiveAddress = _lpReceiver;
            (, , uint liquidity) = _swapRouter.addLiquidity(
                address(this), _usdt, lpAmount, fundUsdt, 0, 0, receiveAddress, block.timestamp
            );
...
contract BOSS is AbsToken {
    ...
    //Total
        1999,
    //Receive
        address(0x8a6C5bf5b3707D7579A510E883e10E95740DDE8B),
    //Fund
        address(0xF877c1B1a317dB36fE27c8fbb5A8ca3c171B2396)
    ){
